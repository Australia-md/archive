...
// ═══════════════════════════════════════════════════════
// 4. HERO STATS — fetch stats.json, then animate counter
// ═══════════════════════════════════════════════════════

/**
 * Fetch /stats.json (generated by generate-stats.js on each job run),
 * update the data-target attribute on the entries counter, then kick
 * off the animated count-up.  Falls back to the hardcoded data-target
 * if the fetch fails (e.g. local dev without a stats.json).
 */
function loadStatsAndInitCounter() {
  ...
  fetch('/stats.json')
    ...
    .catch(() => {
      // Fallback: animate whatever data-target is already in the HTML.
      initStatsCounter();
    });
}
