...
function validateContent(content, template) {
...
async function handleSubmit(event) {
    ...
    try {
        ...
        if (response.status === 429) {
            ...
            showErrorState('RATE_LIMITED', errorBody.error || 'Too many submissions right now. Please try again shortly.', retryAfterSeconds);
            return;
        }
...
function showSuccessState(issueUrl, issueNumber) {
...
function startRateLimitCountdown(retryAfterSeconds) {
    ...
    const updateCountdown = (secondsRemaining) => {
...
function clearStatusBanner(statusBanner) {
    statusBanner.textContent = '';
