...
/**
 * Fetch /stats.json (regenerated by generate-stats.js on each job run),
 * update the data-target on the entries counter, then animate.
 * Falls back to the hardcoded data-target value if the fetch fails.
 */
function loadStatsAndInitCounter() {
    ...
    fetch('/stats.json')
        ...
        if (typeof data.generated === 'string' && data.generated) {
            const updatedEl = document.querySelector('#stat-updated .stat-date');
            if (updatedEl)
                updatedEl.textContent = formatUpdated(data.generated);
        }
    })
        .catch(() => {
