...
function initSearchAutocomplete() {
    ...
    if (!inputEl || !suggestionsBoxEl || !suggestionsListEl || !formEl)
        return;
    const input = inputEl;
    const suggestionsBox = suggestionsBoxEl;
    ...
    input.addEventListener('input', () => {
        ...
        const filtered = SUGGESTIONS_DATA.filter((item) => item.text.toLowerCase().includes(query) ||
            item.category.toLowerCase().includes(query)).slice(0, 6);
